...
; DESCRIPTION: Subtracts one matrix from another element by element, which needs
;              only a single loop because the shape never enters into it.
...
START:
    ...
    ; -------------------------------------------------------------------------
    ; ADDITION AND SUBTRACTION ARE ELEMENTWISE, SO THE MATRIX CAN BE TREATED
    ; AS A FLAT RUN OF NINE VALUES. THE ROWS AND COLUMNS ONLY MATTER WHEN
    ; PRINTING IT.
    ; -------------------------------------------------------------------------
    XOR SI, SI
    MOV CX, CELLS
...
; TECHNICAL NOTES
; =============================================================================
; 1. SHAPE DOES NOT ENTER INTO IT:
;    - Every element is combined with the one in the same place, so one
;    - loop over all nine is enough. Multiplication is the operation that
;    - genuinely needs the two dimensional structure.
; 2. THE RESULT MAY BE NEGATIVE:
;    - Which is why the display prints signed values. Subtracting matrices
;    - of positive numbers routinely produces negative ones.
; = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
